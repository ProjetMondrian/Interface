...
def thirdAlgorithm():
    ...
    np = getNumPoints()
    f = getFacialState()
    syntax = function + currentImage + ' ' + f + ' ' + np
    os.system(syntax)
    img = Image.open('FinalResult.jpg')
    img.resize((500,500),0).save('FinalResult2.jpg')
    img = Image.open('FinalResult2.jpg')
...
algo1Bouton = Button(region1_2, text="Preset 1", command= preset1)
algo1Bouton.pack(side=TOP)

algo1Bouton = Button(region1_2, text="Preset 2", command= preset2)
algo1Bouton.pack(side=TOP)

space = Label(region1_2,text="    ").pack(side=TOP)
txtNombreLignes = Label(region1_2,text="Number of lines").pack(side=TOP)

varNombreLignes = StringVar(fenetre)
varNombreLignes.set("50")
nombreLignes = Spinbox(region1_2, from_= 10, to = 100, increment = 10, textvariable = varNombreLignes)
nombreLignes.pack(side = TOP)

space = Label(region1_2,text="    ").pack(side=TOP)
txtNombreLignes = Label(region1_2,text="Line magnetisme").pack(side=TOP)

varLineMag = StringVar(fenetre)
varLineMag.set("4")
linemag = Spinbox(region1_2, from_= 2, to = 8, increment = 1, textvariable = varLineMag)
linemag.pack(side = TOP)

space = Label(region1_2,text="    ").pack(side=TOP)
txtscale = Label(region1_2,text="Scale").pack(side=TOP)
varTxtScale = StringVar(fenetre)
varTxtScale.set("20")
scale = Spinbox(region1_2, from_= 10, to = 30, increment = 5, textvariable = varTxtScale)
scale.pack(side = TOP)

space = Label(region1_2,text="    ").pack(side=TOP)

space = Label(region1_2,text="Coloration").pack(side=TOP)

option = Listbox(region1_2, selectmode = BROWSE, height = 2)
...
algo1Bouton = Button(region1_2, text="Start Algo 1", command= firstAlgorithm)
algo1Bouton.pack(side=TOP)
space = Label(region1_2,text="    ").pack(side=TOP)
...
parameters2 = Label(region1_2,text="PARAMETERS ALGO 2", bg = 'white').pack(side=TOP)

##space = Label(region1_2,text="    ").pack(side=TOP)

txtscale = Label(region1_2,text="Number of colors").pack(side=TOP)
varNumColors = StringVar(fenetre)
varNumColors.set("5")
numColors = Spinbox(region1_2, from_= 5, to = 20, increment = 5, textvariable = varNumColors)
numColors.pack(side = TOP)

space = Label(region1_2,text="    ").pack(side=TOP)

varMC = IntVar()
mColor = Checkbutton(region1_2, text="Mondrian colors only", variable = varMC)
mColor.pack(side = TOP)


algo1Bouton = Button(region1_2, text="Start Algo 2", command= secondAlgorithm)
algo1Bouton.pack(side=TOP)

space = Label(region1_2,text="    ").pack(side=TOP)
space = Label(region1_2,text="    ").pack(side=TOP)

space = Label(region1_2,text="PARAMETERS ALGO 3", bg = 'white').pack(side=TOP)

##space = Label(region1_2,text="    ").pack(side=TOP)
txtscale = Label(region1_2,text="Number of points").pack(side=TOP)
varNumPoints = StringVar(fenetre)
varNumPoints.set("200")
numPoints = Spinbox(region1_2, from_= 50, to = 500, increment = 50, textvariable = varNumPoints)
numPoints.pack(side = TOP)

varFR = IntVar()
facial = Checkbutton(region1_2, text="Facial recognition (ignores number of points)", variable = varFR)
facial.pack(side = TOP)

algo1Bouton = Button(region1_2, text="Start Algo 3", command= thirdAlgorithm)
algo1Bouton.pack(side=TOP)
...
space2 = Label(region1_2, text = "    ").pack(side=TOP)

quitButton = Button(region1_2, text="Quit", command = close_window).pack(side=BOTTOM)
